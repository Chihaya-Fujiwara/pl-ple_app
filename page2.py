...
def page2(genre,X,Y,p2,save_dir):
    ...
    with col2.expander("File setting"):
        ...
        extention = st.selectbox('拡張子',('png','jpg','tiff'))
    ...
    col1.pyplot(fig)
    fig.savefig('fig/template.'+str(extention),dpi=350,bbox_inches='tight')
        
    with open('fig/template.'+str(extention), "rb") as file:
        st.download_button(
                label="Download image",
                data=file,
                file_name='fig/template.'+str(extention),
                mime="image/tiff/png",
            )
