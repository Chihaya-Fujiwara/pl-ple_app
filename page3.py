...
def page3(genre,df5,wb,save_dir):
    ...
    if sele == "Emission":
        ...
        with col1.expander("Save"):
            ...
            data = convert.convert_df(data)
            
            if 'csv' in sv_opt:    
                st.download_button(
                label="Download spctrum file as CSV",
                data= data,
            ...
            if 'Emission' in sv_opt:
                fig1[0].savefig('fig/em_sp.tiff',dpi=350,bbox_inches='tight')
                
                with open('fig/em_sp.tiff', "rb") as file:
                    st.download_button(
                            label="Download image",
                            data=file,
                            file_name='fig/em_sp.tiff',
                            mime="image/tiff/png",
                        )
            elif 'Excitation' in sv_opt:                      
                fig2[0].savefig('fig/ex_sp.tiff',dpi=350,bbox_inches='tight')
                
                with open('fig/ex_sp.tiff', "rb") as file:
                    st.download_button(
                            label="Download image",
                            data=file,
                            file_name='fig/ex_sp.tiff',
                            mime="image/tiff/png",
                        )    
            #if 'Both' in sv_opt:                      
            else:
                fig3[0].savefig('fig/both_sp.tiff',dpi=350,bbox_inches='tight')
                with open('fig/both_sp.tiff', "rb") as file:
                    st.download_button(
                            label="Download image",
                            data=file,
                            file_name='fig/both_sp.tiff',
                            mime="image/tiff/png",
                        )  
